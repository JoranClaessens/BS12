        ...
        public string EncryptText(string verzender, string ontvanger, string encryptieFilePath, string boodschap)
        {
            ...
            try
            {
                ...
                using (AesManaged myRijndael = new AesManaged())
                {
                    ...
                    var fileId = (from bestanden in BS12.GebruikerFiles
                                  select bestanden).Count();

                    string userFile = System.IO.Path.Combine(filesFolder, ontvanger + "_" + (fileId + 1) + ".zip");

                    ZipFile.CreateFromDirectory(editFolder, userFile);

                    var gebruikersId = (from gebruiker in BS12.Gebruikers
                                        where gebruiker.Gebruikersnaam == verzender
                                        select gebruiker.Id).First();

                    GebruikerFile bestand = new GebruikerFile();
                    bestand.FileId = fileId + 1;
                    bestand.gebruikersId = gebruikersId;
                    BS12.GebruikerFiles.Add(bestand);
                    BS12.SaveChanges();
                }
            }
            catch (Exception e)
